...
*/
 array<string ,19 /* you must write the number string */ >herold = { "Rose","Raily","fuoscoo" }; 
array<string , 12> Hi= { "ali","SLash","Man"};

cout<<herold.front() <<endl;
cout<<herold.back() <<endl; // don't print enyting
cout<<herold.empty() <<endl;
cout<<herold.size() <<endl;
cout<< sizeof(herold) <<endl;
cout<<sizeof (herold[2]) <<endl;
cout<<herold[2]<<endl;
...
//cout << strcat( (G[0]), (S[0])) <<endl; //error : I don't know
...
int Messi []={ 1, 2, 3 , 4 ,5 ,69 ,88 ,565 };
for (int B = 7 ; B >= 2 ;  )
{
if ( Messi [B] == 5)
{
  break;  // the opreration will stop
}
